ActiveAdmin.register Order, as: 'constructing' do
  menu priority: 4, label: -> { "製作中" }
  ...
  member_action :upgrade_status, method: :post do
    ...
    @order.done!
    if @order.save
      redirect_to admin_done_path(@order)
    end
  end
  ...
  index :title => "製作中" do
    ...
    column "注文番号" do |order|
      link_to order.id, admin_constructing_path(order), class: :narrow
    end
    column "種別" do |order|
      order.customer_is_company ? "法人" : "個人"
    end
    column :status, sortable: false do |order|
      span class: :status_constructing do
  ...
  show do
    render "admin/constructings/show"
  end
end
