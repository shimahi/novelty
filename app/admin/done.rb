ActiveAdmin.register Order, as: 'done' do
  menu priority: 5, label: -> { "発送済み" }
  ...
  controller do
    ...
    def show
      ...
      @page_title = "発送済み注文 #"+ @order.id.to_s
    end
  end
  ...
  index :title => "発送済み" do
    ...
    column "注文番号" do |order|
      link_to order.id, admin_done_path(order), class: :narrow
    end
    column "種別" do |order|
      order.customer_is_company ? "法人" : "個人"
    end
    column :status, sortable: false do |order|
      span class: :status_done do
